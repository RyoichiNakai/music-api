...
- 今回のテーブル設計では、アルバムと曲のリレーションを追加していない
  - クエリを必要としていなかったため
- 複数のリクエストパラメータ使用時には、参考サイトのように、SKを{エンティティ名}-{複数リクエストパラメータ}としておく
  - 例：Song_ArtistName_Release
  - こうしないと、実装がしんどくなる
  - 複数回、探索をしないといけなくなるので、遅延が大きく生じる
  - 結果、いいことがない
- ジャンル別のアルバムを取得する際のインデックスについて
  - album_genre -> album-uuid から album-uuid -> album_nameと検索をかける
  - album-uuid -> album_nameをGSIを使用するのと使用しないで`約600ms`の遅延が生じる
